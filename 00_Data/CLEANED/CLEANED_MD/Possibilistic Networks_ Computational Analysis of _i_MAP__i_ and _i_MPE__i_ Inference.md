...
- We denote by $\mathbf{B} \& \mathbf{B}_{\otimes}-\mathbf{D}-\mathbf{M A P}\left(\mathcal{P N}_{B \& B_{\otimes}}, Q, e\right)$ the decision problem associated to $M A P$ querying a binary and boolean possibilistic network.
...
To show hardness and completeness of $M A P$ and $M P E$ queries, we will provide polynomial-time reductions from some known $N P$-complete problems to our MAP decision problems (resp. MPE decision problems) and conversely.
