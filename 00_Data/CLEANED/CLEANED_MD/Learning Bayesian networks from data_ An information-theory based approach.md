...
$A:=A^{\prime} \quad \%$ then remove $\langle X, Y\rangle$ from $A$
4. Return $(V, A)$
...
6. While $|C|>1$ do
(a) For each $i$, let $C_{i}=C \backslash\{$ the $i$ th node of $C\}, s_{i}=I_{D}\left(X, Y \mid C_{i}\right)$.
(b) Let $m=\operatorname{argmin}_{i}\left\{s_{1}, s_{2}, \ldots\right\}$.
(c) If $s_{m}<\varepsilon, \quad \% s_{m}=\min \left(s_{1}, s_{2}, \ldots\right)$.
...
Subroutine TPDA (D: Dataset, \(\varepsilon\) : threshold): returns \(\boldsymbol{G}=(V, E)\) : graph structure
            Begin [Drafting].
1. Let \(V=\{\operatorname{attributes}\) in \(\boldsymbol{D}\}, E=\{\}\)
...
# 5.3. TPDA-П algorithm for learning, given ordering 
