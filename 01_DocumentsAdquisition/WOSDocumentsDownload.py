...
logger = setup_logger(
    ...
    general_log_filename='WOSDocumentsDownload.log',
    error_log_filename='WOSDocumentsDownload_error.log',
...
def download_article_with_fallback(row, email, output_dir):
    """
    ...
    Parameters:
        row (dict): Dictionary containing article metadata (DOI, title, etc.).
    ...
    title = row.get("Article Title", "untitled_article")
    logger.info(f"Processing: {title[:50]}{'...' if len(title) > 50 else ''} (DOI: {row.get('DOI')})")

    try:
        # Attempt Unpaywall
        logger.info(f"  Querying Unpaywall API...")
        pdf_url, is_direct_pdf = query_unpaywall(row["DOI"], email)
        ...
        if result["Reason"] in ["Success", "File already exists"]:
            ...
            if not os.path.exists(json_path):
                try:
                    api_url = f"https://api.unpaywall.org/v2/{row['DOI']}?email={email}"
                    response = requests.get(api_url, timeout=10)
                    response.raise_for_status()
                    data = response.json()
...
def main():
    ...
    try:
        # Load the Excel file containing article metadata
        logger.info("Loading article metadata from Excel file...")
        excel_path = CSV_DOI_PATH
        ...
        # Get the user's email for API access
        email_input = EMAIL
        ...
        for index, row in df.iterrows():
            ...
            try:
                ...
                if not row.get("DOI"):
                    logger.warning(f"  Missing DOI for article: {row.get('Article Title', 'Unknown Title')}")
                    failed_downloads.append({
                        "Index": index,
                        "DOI": "Missing",
                        "Article Title": row.get("Article Title", "Unknown"),
                        "Status": "Failed",
                        "Reason": "Missing DOI"
                    })
                    continue
        ...
        if failed_downloads:
            ...
            failed_path = os.path.join(failed_dir, 'failed_downloads.csv')
