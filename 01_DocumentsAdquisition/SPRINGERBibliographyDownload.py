...
logger = setup_logger(
    ...
    general_log_filename='SPRINGERBibliographyDownload.log',
    error_log_filename='SPRINGERBibliographyDownload_error.log',
...
def extract_titles(bibliography: List[str]) -> List[str]:
    ...
    for entry in bibliography:
        ...
        if match:
            # Try to extract the first non-empty group
            title = next((g.strip() for g in match.groups() if g and g.strip()), None)
            if title:
                titles.append(title)
...
def main():
    ...
    input_file = BIBLIOGRAPHY_SPRINGER_FILE
    output_file = os.path.join(BIBLIOGRAPHY_TITLES_FOLDER, BIBLIOGRAPHY_SPRINGER_FILE.split('/')[-1].replace('.txt', '_titles.txt'))
    email_input = EMAIL
